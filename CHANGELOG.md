...
- `skills/research-driven-development/researcher-prompt.md` — Researcher subagent prompt template (like `implementer-prompt.md`). Contains full researcher workflow, research principles, output format, and report format. Named after Jesse Vincent (superpowers creator). Replaces standalone `jesse.md` agent file — the skill owns the prompt.
