...
| **Hermes plugin tests** (`tests/hermes/`) | ships `tests/hooks/test-hermes-injection.sh` — bash driving `python3`, asserting the anti-fork properties (exactly one composer invocation, no selection-policy tokens, `shell=False` + timeout) plus behavioural checks against a fake `ctx` | ships a **pytest** suite under `tests/hermes/` (conftest, test_bootstrap, test_plugin) | the plugin under test differs by the beads delta — upstream builds its bootstrap inline in Python, we exec `hooks/session-start --emit-plain` — so the suites necessarily assert different things. Framework choice is ours: this repo's hook-test surface already mixes `.sh` and `.mjs` under `just hooks`, so bash adds no dependency or runner while pytest adds both. Registered because ADR-0048:31 classifies "our mechanism differs from upstream's for the same job" as drift-to-fix unless it IS the beads delta, and a test framework is not. Mark SKIP, not Conflict |
