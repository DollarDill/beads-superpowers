...
| **Hermes Agent harness** (`.hermes-plugin/`) | not shipped — Devin CLI IS adopted (manifest-only, Best-effort tier), Hermes is deliberately deferred | ships `.hermes-plugin/plugin.yaml` + `__init__.py` (104 lines of Python), `references/hermes-tools.md`, and a pytest suite under `tests/hermes/` | upstream's Hermes `__init__.py` is a full bootstrap injector (`pre_llm_call`) that injects ONLY the stripped `using-superpowers` body. This fork's `hooks/session-start` also composes beads context (curated memories + a `bd prime` pointer), so adopting upstream's verbatim would ship a harness missing the beads half of the product. Doing it properly means a SECOND implementation of load-bearing bootstrap logic in a second language, on a path that already produced a P0 (unredacted memory keys on the python3-absent degraded path). Also brings a YAML manifest dialect and pytest, neither of which this fork carries. Deferred to its own spec, not rejected. Mark SKIP, not Conflict |

When a CHANGED skill from Phase 5 matches a row here, mark it **SKIP (deliberate divergence)** in the report — not drift.

## The Audit Process

You MUST create an audit bead and complete ALL 9 phases in order:
...
> first, re-stamp second.

**Checks 9.1 and 9.2** — the per-path disposition buckets, the hunk-coverage measurement, and the
scaling rules for a multi-release delta — live in
[references/check-suite.md](references/check-suite.md). Open it when you reach this phase.
...
Done when: a **per-path disposition table** has been produced and recorded in the audit report —
every path in `OLD..NEW` in exactly one bucket with its evidence named — every adopted `skills/`
file has been hunk-checked against upstream `NEW`, and only then are the baselines re-stamped.
...
| Trigger | Action |
|---------|--------|
| Before any plugin release | Full audit (all 9 phases) — MANDATORY |
