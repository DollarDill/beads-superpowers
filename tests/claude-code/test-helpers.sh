...
# Case-insensitive: these assertions match free-form model prose where
# capitalization varies run to run (e.g. "Do Not Trust" vs "not trust").
assert_contains() {
    local output="$1"
    local pattern="$2"
...
create_test_plan() {
    ...
    local plan_file="$project_dir/.internal/plans/$plan_name.md"
