...
grep -qi "beads not found" <<<"$out" || { echo "FAIL: no bd-absent hint"; fail=1; }
...
grep -qi "obra/superpowers appears installed" <<<"$out" || { echo "FAIL: no collision warning from installed_plugins.json"; fail=1; }
rm -rf "$tmp"
...
grep -qi "obra/superpowers appears installed" <<<"$out" || { echo "FAIL: no collision warning from superpowers-marketplace handle"; fail=1; }
rm -rf "$tmp"
...
grep -qi "obra/superpowers appears installed" <<<"$out" && { echo "FAIL: false-positive collision warning"; fail=1; }
rm -rf "$tmp"
...
grep -qi "Permission denied" <<<"$out" && { echo "FAIL: hook output contains 'Permission denied'"; fail=1; }
grep -qi "cat:" <<<"$out" && { echo "FAIL: hook output contains 'cat:'"; fail=1; }
