...
#    `bd kv list` TEXT output. bd renders each pair as `  key = value` with a TWO-SPACE indent, so
#    the skim pattern MUST tolerate leading whitespace (`^ *`); a column-0 `^bsp.kb` anchor matches
#    nothing against real output. Positive + negative controls together prove the pattern is live AND
#    that a multi-line value is not silently retrievable.
