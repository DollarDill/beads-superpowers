...
| Inter-agent misalignment is a leading multi-agent failure class (MAST); single-threaded writes avoid it structurally (Cognition) | Only the orchestrator creates, claims, or closes beads; subagents hand off through files, not a live conversation | Orchestrator-only design, `subagent-driven-development` |
