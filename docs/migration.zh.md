...
四个理念决定了这个知识库是否值得搭建。搞错它们，你之后就得重建；本仓库就重建过两次，本阶段末尾的证据框正是原因。
...
在同一次审查中，标记出任何其内容不应到达共享 Dolt 远端的东西——内部策略、未发布的计划、竞争定位。迁移会同步描述，这是源文件从未有过的新暴露。把被标记的条目搁置：让它们不进知识库，记录到一个对账守卫会尊重的 gitignored 排除清单里，只有当你拥有一个私有远端来存放它们之后再迁移。
...
!!! example "在本仓库——129 个 kv + 36 篇文档 + 55 个 ADR"
    三个语料迁入 216 个知识 bead：63 个 decision、75 个 design、78 个 research。键值存储退休为带每键溯源的墓碑；ADR 文件和研究文档原地保留，作为其 bead 所指向的深入正文。有八个条目——六篇研究文档和两个 ADR——在审查期间被搁置，因为其内容尚不应到达共享远端，由一份 gitignored 排除清单保持守卫为绿。每一遍运行都幂等：第二遍创建零个 bead。密钥扫描在一个形似令牌的文件名落地前将其标出，未写入任何敏感内容。
