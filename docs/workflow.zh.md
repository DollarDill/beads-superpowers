...
当一个选择难以逆转、离开上下文令人困惑、且存在真实权衡时，智能体会提出在 `docs/decisions/` 目录中记录一条 ADR——包含背景、决策、后果和已考虑的替代方案。只要某个决策大体符合这些标志，智能体便倾向于提议记录；只有例行澄清和范围确认才排除在外。它将隐式决策转化为后续读者可以追溯的显式记录。
