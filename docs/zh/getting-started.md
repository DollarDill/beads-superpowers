...
| Hermes Agent | `hermes plugins install DollarDill/beads-superpowers` | — | 原生注册技能，并通过 `pre_llm_call` 注入会话引导内容（含 beads 上下文）；配置已由我们验证，未经端到端测试 |
...
Dolt 历史会保留已删除的行，因此若远端与你的代码仓库相同，会连带把这整段历史一并公开。一个专属的私有仓库能让 issue 数据保持鉴权访问，同时代码仍可保持公开。目前没有任何机制阻止你把 bd 指向代码仓库——bd 有一道碰撞防护（URL 与 git origin 相同时，`bd dolt remote add` 会拒绝执行，除非传入 `--allow-git-origin`），但它尚未进入任何已发布版本（2026-08-15 重新核查至 v1.2.2——该版本只是把 v1.1.2 的代码以更高版本号重新发布）。请把"使用独立远端"当作你自己的纪律，不要以为升级之后该防护就已生效。
