...
sequenceDiagram
  ...
  alt bd prime 已在别处注册
    ...
    SH->>BD: 请求精选记忆、最新线索指针、知识库指针
    BD-->>SH: 按显著度精选的记忆与指针
    SH->>SH: 组合技能引导 + 精选记忆 + 线索指针 + 知识库指针
    SH-->>A: 注入组合后的上下文
  end
  Note over A: 智能体已就绪定位：技能已加载，此前状态尽在眼前
```
...
如果你要把已有的一堆 ADR、wiki 或研究笔记搬进这个存储，而不是从空仓库开始，请见[迁移指南](migration.md)。
