...
**KB document workflow:** Write research documents to `./docs/research` (override per-project via `bd config set custom.research-output-dir`, or globally via `RESEARCH_OUTPUT_DIR` env var). DCI (`!` backtick syntax) does not work in agent `.md` files — only in `SKILL.md` and `.claude/commands/*.md`. The research-driven-development skill handles dynamic resolution via its own DCI; this agent uses the default path. Search for existing coverage first using `bd memories <keyword>` and searching the research directory. After writing, commit the document.
