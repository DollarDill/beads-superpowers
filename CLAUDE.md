...
- **Remote swaps: collision guard + stale `sync.remote`** — a git-origin collision guard makes `bd dolt remote add` refuse a URL matching git origin unless `--allow-git-origin` is passed (upstream PR #4153, merged 2026-07-04, hours after the v1.1.0 tag). **No released bd carries it yet** — verified 2026-07-27 against v1.1.2: v1.1.1 never shipped (tag cut, release pipeline failed on a stale MCP lockfile, version burned) and v1.1.2 was a hotfix cut from the v1.1.0 tag rather than from main. Re-verified 2026-08-15 against the newest release: **v1.2.2 still does not carry it**, because v1.2.2 re-releases the v1.1.2 code under a higher version number (v1.2.0/v1.2.1 were accidental and are retracted). So do NOT assume the guard is active after upgrading — no published bd has it, and the next release to cut from `main` is the first that could. For a deliberate same-repo setup, pass the flag rather than fighting it. Separately, `bd dolt remote remove` can leave `sync.remote` stale or commented out in `.beads/config.yaml` — verify it after any swap and repair with `bd config set sync.remote <url>` if it's wrong.
