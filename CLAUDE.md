...
- **bd v1.2.0 and v1.2.1 are POISONED — never install them** — both were published by accident on 2026-08-11 without release testing, and `go.mod` retracts them (along with v1.1.1). Running either binary **once** migrates the local Dolt schema **v53 → v65**, after which every other bd binary refuses to start: `schema version mismatch: database is at v65, binary knows up to v53`. Recovery is ~2 minutes — [`docs/RECOVERY-1.2.1.md`](https://github.com/gastownhall/beads/blob/v1.2.2/docs/RECOVERY-1.2.1.md): roll the schema cursor back to v53 with one `dolt sql` command, or use `BD_IGNORE_SCHEMA_SKEW=1 bd <command>` as a verified stopgap. **Upgrade every machine and clone to a safe version BEFORE recovering**, or a leftover 1.2.1 binary silently re-migrates the database. **Safe versions: v1.1.2 or v1.2.2.** Note v1.2.2 is *not* an upgrade in capability — it re-releases the tested v1.1.2 code under a higher version number, so it carries **none** of the 1.2.x features (work leases, events journal, sync federation, HTTP API server, provenance events). A bd version bump is not evidence of a capability change.
- **Zero-remote `bd dolt push` silently adopts git origin (bd v1.1.0 – v1.2.2)** — on those versions, with NO Dolt remote configured, `bd dolt push` does not error; it silently pushes the beads DB to the git origin instead (a real, reproduced ADR-0057 incident). **Fixed in v1.3.0** (upstream #5068, verified 2026-09-16 against the v1.3.0 binary): adoption now prompts (defaulting to no) and **fails closed non-interactively**, which is every agent shell; `--yes` consents ahead of time, and `--no-adopt` / `BD_NO_REMOTE_ADOPT=1` disables adoption entirely and wins over `--yes`. Note the trap covers v1.2.2 as well, since v1.2.2 is the v1.1.2 tree under a higher version number. Whatever the version, always chain remote swaps in one command — `bd dolt remote remove origin && bd dolt remote add origin <url>` — never leave the remote empty between the two.
- **Remote swaps: collision guard + stale `sync.remote`** — a git-origin collision guard makes `bd dolt remote add` refuse a URL matching git origin unless `--allow-git-origin` is passed (upstream PR #4153, merged 2026-07-04, hours after the v1.1.0 tag). **No released bd carries it yet** — verified 2026-07-27 against v1.1.2: v1.1.1 never shipped (tag cut, release pipeline failed on a stale MCP lockfile, version burned) and v1.1.2 was a hotfix cut from the v1.1.0 tag rather than from main. Re-verified 2026-08-15 against the newest release: **v1.2.2 still does not carry it**, because v1.2.2 re-releases the v1.1.2 code under a higher version number (v1.2.0/v1.2.1 were accidental and are retracted). **Resolved in v1.3.0** — verified 2026-09-16 against the v1.3.0 binary (`bd dolt remote add --help` lists `--allow-git-origin`). v1.3.0 is the release that prediction was waiting on: the first cut from `main` since the 1.1 line. So the guard is live on v1.3.0+ and on nothing earlier. For a deliberate same-repo setup, pass the flag rather than fighting it. Separately, `bd dolt remote remove` can leave `sync.remote` stale or commented out in `.beads/config.yaml` — verify it after any swap and repair with `bd config set sync.remote <url>` if it's wrong.
