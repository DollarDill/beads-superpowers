...
- Before proposing any design, invoke the `knowledge-retrieval` skill with the design topic and why you are asking (design grounding). If a prior decision already covers this, surface it rather than re-litigating.
- If the skill is unavailable: `bd list --label <topic> --status all` + `bd search "<keywords>" --status all` (body terms need `--desc-contains "<term>"` — search matches titles only; >10 hits: narrow the query, never triage truncated titles) + `bd memories <keyword>` — hits are pointers, not knowledge; read hit bodies with `bd show <ids>` / `bd recall <key>` before relying on them (full bodies, never truncated below 50 lines; distilled <=2.5KB by design — sanctioned spend). 0 relevant does not mean none exist — re-angle the query once before emitting `KB check: none`. Emit `KB check (fallback): N bead hits, M memory hits, K read` (or `KB check: none`) plus one disposition line per hit: folded in (what it changed) or ruled out (why). The check is complete when every plausibly-relevant hit is dispositioned, never when a count is reached.
